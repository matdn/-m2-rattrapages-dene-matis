...
export default function HomeScreen({ navigation }) {
    ...
    const [sortMode, setSortMode] = useState('rating'); // 'rating' | 'swell' | 'wind'
    ...
    const onPressSpot = (spot) => {
        // Si vous avez React Navigation, naviguez vers un écran "Details"
        // navigation?.navigate('Details', { spotId: spot.id });
        // Pour l'instant, on affiche juste une alerte légère (remplacez par Toast si besoin)
        console.log('Spot sélectionné:', spot.id);
    };
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                renderItem={({ item }) => (
                    <Pressable onPress={() => onPressSpot(item)}>
                        <SpotCard item={item} />
                    </Pressable>
                )}
...
function Footer() {
    return (
        <View style={styles.footer}>
            <Text style={styles.footerText}>Données factices — modifiez RAW_SPOTS dans HomeScreen.js</Text>
        </View>
    );
}
